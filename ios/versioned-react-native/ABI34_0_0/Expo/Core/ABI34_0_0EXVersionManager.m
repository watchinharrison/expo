...
#import "ABI34_0_0EXAppState.h"
#import "ABI34_0_0EXDevSettings.h"
#import "ABI34_0_0EXDisabledDevLoadingView.h"
#import "ABI34_0_0EXDisabledDevMenu.h"
#import "ABI34_0_0EXDisabledRedBox.h"
#import "ABI34_0_0EXFileSystem.h"
#import "ABI34_0_0EXVersionManager.h"
#import "ABI34_0_0EXScopedBridgeModule.h"
#import "ABI34_0_0EXStatusBarManager.h"
#import "ABI34_0_0EXUnversioned.h"
#import "ABI34_0_0EXScopedFileSystemModule.h"
#import "ABI34_0_0EXTest.h"
...
#import <ReactABI34_0_0/ABI34_0_0RCTExceptionsManager.h>
...
#import <ReactABI34_0_0/ABI34_0_0RCTAsyncLocalStorage.h>
...
#import "ABI34_0_0EXScopedModuleRegistryDelegate.h"

// used for initializing scoped modules which don't tie in to any kernel service.
#define ABI34_0_0EX_KERNEL_SERVICE_NONE @"ABI34_0_0EXKernelServiceNone"

// this is needed because ABI34_0_0RCTPerfMonitor does not declare a public interface
// anywhere that we can import.
@interface ABI34_0_0RCTPerfMonitorDevSettingsHack <NSObject>
...
static NSMutableDictionary<NSString *, NSDictionary *> *ABI34_0_0EXScopedModuleClasses;
void ABI34_0_0EXRegisterScopedModule(Class, ...);
void ABI34_0_0EXRegisterScopedModule(Class moduleClass, ...)
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    ABI34_0_0EXScopedModuleClasses = [NSMutableDictionary dictionary];
  });
  ...
  va_start(argumentList, moduleClass);
    while ((kernelServiceClassName = va_arg(argumentList, NSString*))) {
      if ([kernelServiceClassName isEqualToString:@"nil"]) {
        unversionedKernelServiceClassNames[kernelServiceClassName] = ABI34_0_0EX_KERNEL_SERVICE_NONE;
      } else {
  ...
  if (moduleClassName) {
    ABI34_0_0EXScopedModuleClasses[moduleClassName] = unversionedKernelServiceClassNames;
  }
}

@interface ABI34_0_0RCTBridgeHack <NSObject>

- (void)reload;

@end

@interface ABI34_0_0EXVersionManager ()
...
@implementation ABI34_0_0EXVersionManager
...
{
  // manually send a "start loading" notif, since the real one happened uselessly inside the ABI34_0_0RCTBatchedBridge constructor
  [[NSNotificationCenter defaultCenter]
   postNotificationName:ABI34_0_0RCTJavaScriptWillStartLoadingNotification object:bridge];
}
...
{
  ABI34_0_0RCTDevSettings *devSettings = [self _moduleInstanceForBridge:bridge named:@"DevSettings"];
...
#ifdef ABI34_0_0EX_ENABLE_UNSAFE_SYSTRACE
    items[@"dev-profiler"] = @{
...
- (void)selectDevMenuItemWithKey:(NSString *)key onBridge:(id)bridge
{
  ...
  if ([key isEqualToString:@"dev-reload"]) {
    // bridge could be an ABI34_0_0RCTBridge of any version and we need to cast it since ARC needs to know
    // the return type
    [(ABI34_0_0RCTBridgeHack *)bridge reload];
...
- (void)showDevMenuForBridge:(id)bridge
{
  ...
  if ([devMenu respondsToSelector:@selector(show)]) {
    [((ABI34_0_0RCTDevMenu *)devMenu) show];
  }
}
...
{
  return ([ABI34_0_0RCTGetURLQueryParam([bridge bundleURL], @"dev") boolValue]);
}

- (id<ABI34_0_0RCTBridgeModule>)_moduleInstanceForBridge:(id)bridge named:(NSString *)name
{
  if ([bridge respondsToSelector:@selector(batchedBridge)]) {
    bridge = [bridge batchedBridge];
  }
  ABI34_0_0RCTModuleData *data = [bridge moduleDataForName:name];
  if (data) {
...
- (void)configureABIWithFatalHandler:(void (^)(NSError *))fatalHandler
                        ...
                        logThreshold:(NSInteger)threshold
{
  ABI34_0_0RCTSetFatalHandler(fatalHandler);
  ABI34_0_0RCTSetLogThreshold(threshold);
  ABI34_0_0RCTSetLogFunction(logFunction);
}
 ...
 *    @ABI34_0_0EXTestEnvironment testEnvironment
 *    NSDictionary *services
 *
 * Kernel-only:
 *    ABI34_0_0EXKernel *kernel
...
- (NSArray *)extraModulesWithParams:(NSDictionary *)params
{
  ...
  NSMutableArray *extraModules = [NSMutableArray arrayWithArray:
                                  @[
                                    [[ABI34_0_0EXAppState alloc] init],
                                    [[ABI34_0_0EXDevSettings alloc] initWithExperienceId:experienceId isDevelopment:(!isOpeningHomeInProductionMode && isDeveloper)],
                                    [[ABI34_0_0EXDisabledDevLoadingView alloc] init],
                                    [[ABI34_0_0EXStatusBarManager alloc] init],
                                    ]];
  ...
  if (exceptionsManagerDelegate) {
    ABI34_0_0RCTExceptionsManager *exceptionsManager = [[ABI34_0_0RCTExceptionsManager alloc] initWithDelegate:exceptionsManagerDelegate];
    [extraModules addObject:exceptionsManager];
  } else {
    ABI34_0_0RCTLogWarn(@"No exceptions manager provided when building extra modules for bridge.");
  }
  
  if (params[@"testEnvironment"]) {
    ABI34_0_0EXTestEnvironment testEnvironment = (ABI34_0_0EXTestEnvironment)[params[@"testEnvironment"] unsignedIntegerValue];
    if (testEnvironment != ABI34_0_0EXTestEnvironmentNone) {
      ABI34_0_0EXTest *testModule = [[ABI34_0_0EXTest alloc] initWithEnvironment:testEnvironment];
  ...
  if ([params[@"isStandardDevMenuAllowed"] boolValue] && isDeveloper) {
    [extraModules addObject:[[ABI34_0_0RCTDevMenu alloc] init]];
  } else {
    // non-kernel, or non-development kernel, uses expo menu instead of ABI34_0_0RCTDevMenu
    [extraModules addObject:[[ABI34_0_0EXDisabledDevMenu alloc] init]];
  }
  if (!isDeveloper) {
    // user-facing (not debugging).
    // additionally disable ABI34_0_0RCTRedBox
    [extraModules addObject:[[ABI34_0_0EXDisabledRedBox alloc] init]];
  }

  ABI34_0_0UMModuleRegistryProvider *moduleRegistryProvider = [[ABI34_0_0UMModuleRegistryProvider alloc] initWithSingletonModules:params[@"singletonModules"]];

  Class resolverClass = [ABI34_0_0EXScopedModuleRegistryDelegate class];
  ...
  id<ABI34_0_0UMModuleRegistryDelegate> moduleRegistryDelegate = [[resolverClass alloc] initWithParams:params];
  [moduleRegistryProvider setModuleRegistryDelegate:moduleRegistryDelegate];

  ABI34_0_0EXScopedModuleRegistryAdapter *moduleRegistryAdapter = [[ABI34_0_0EXScopedModuleRegistryAdapter alloc] initWithModuleRegistryProvider:moduleRegistryProvider];
  ABI34_0_0UMModuleRegistry *moduleRegistry = [moduleRegistryAdapter moduleRegistryForParams:params forExperienceId:experienceId withKernelServices:services];
  NSArray<id<ABI34_0_0RCTBridgeModule>> *expoModules = [moduleRegistryAdapter extraModulesForModuleRegistry:moduleRegistry];
  [extraModules addObjectsFromArray:expoModules];

  id<ABI34_0_0UMFileSystemInterface> fileSystemModule = [moduleRegistry getModuleImplementingProtocol:@protocol(ABI34_0_0UMFileSystemInterface)];
  NSString *localStorageDirectory = [fileSystemModule.documentDirectory stringByAppendingPathComponent:@"RCTAsyncLocalStorage"];
  [extraModules addObject:[[ABI34_0_0RCTAsyncLocalStorage alloc] initWithStorageDirectory:localStorageDirectory]];
...
- (NSArray *)_newScopedModulesWithExperienceId: (NSString *)experienceId services:(NSDictionary *)services params:(NSDictionary *)params
{
  ...
  if (ABI34_0_0EXScopedModuleClasses) {
    [ABI34_0_0EXScopedModuleClasses enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull scopedModuleClassName, NSDictionary * _Nonnull kernelServiceClassNames, BOOL * _Nonnull stop) {
      ...
      for (id kernelServiceClassName in kernelServiceClassNames) {
        ...
        id service = ([kernelSerivceName isEqualToString:ABI34_0_0EX_KERNEL_SERVICE_NONE]) ? [NSNull null] : services[kernelSerivceName];
