...
class DevMenuButton extends React.PureComponent {
    ...
    renderButtonIcon(icon, isEnabled) {
        ...
        return (<View style={styles.buttonIcon}>
        <MaterialCommunityIcons name={icon} size={20} color={isEnabled ? '#2F9BE4' : '#888'}/>
      </View>);
    }
    renderLabel(label, enabled) {
        if (enabled) {
            return (<StyledText style={styles.buttonText} lightColor="#595c68">
          {label}
        </StyledText>);
        }
        else {
            return (<StyledText style={styles.buttonText} lightColor={LIGHT_DISABLED_TEXT_COLOR} darkColor={DARK_DISABLED_TEXT_COLOR}>
          {label}
        </StyledText>);
        }
    }
    renderDetail(detail) {
        return (<StyledText style={[styles.buttonText, styles.buttonDetailsText]} darkColor={DARK_DISABLED_TEXT_COLOR} lightColor={LIGHT_DISABLED_TEXT_COLOR}>
    ...
    render() {
        ...
        return (<TouchableOpacity style={styles.button} onPress={this.onPress} disabled={!isEnabled}>
