...
class DevMenuTaskInfo extends React.PureComponent {
    _maybeRenderDevServerName() {
        ...
        const devServerName = task && task.manifest && task.manifest.developer ? task.manifest.developer.tool : null;
        if (devServerName) {
            return (<View style={styles.taskDevServerRow}>
    ...
    render() {
        ...
        return (<View style={styles.taskMetaRow}>
        <View style={styles.taskIconColumn}>
          {iconUrl ? (<Image source={{ uri: iconUrl }} style={styles.taskIcon}/>) : (<View style={[styles.taskIcon, { backgroundColor: '#eee' }]}/>)}
        </View>
        <View style={styles.taskInfoColumn}>
          ...
          {sdkVersion && (<StyledText style={styles.taskSdkVersion}>
